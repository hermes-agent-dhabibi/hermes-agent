...
# StreamConsumerConfig agree on the out-of-the-box edit rhythm.  Keep edits
# responsive by default; platforms that need stricter throttling can override
# this in their runtime config.
DEFAULT_STREAMING_EDIT_INTERVAL: float = 0.3
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                if not bridged and not platform_direct and not enabled_was_explicit and not has_channel_overrides:
                    continue
                ...
                if not isinstance(extra, dict):
                    ...
                    plat_data["extra"] = extra
                if enabled_was_explicit:
                    # Preserve explicit disablement for registry-driven plugin
                    # platforms instead of re-enabling them on token presence.
