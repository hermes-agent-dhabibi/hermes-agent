...
def _assert_block_section_header(line: str, section_path: List[str]) -> None:
    """Reject populated inline sections before line-oriented insertion.

    Empty ``{}`` and ``[]`` values are expanded by ``_expand_inline_empty``.
    Any other value means the section is represented in flow/scalar style, so
    inserting indented children beneath it would produce invalid YAML.
    """
    ...
    if value:
        ...
        raise ValueError(
            f"Cannot safely sync config section '{dotted}' because it uses "
            "an inline YAML value. Convert that section to block style first."
        )
    ...
    for depth, key in enumerate(section_path):
        ...
        for i in range(search_start, len(lines)):
            if header_pattern.match(lines[i]):
                ...
                _assert_block_section_header(lines[i], section_path[: depth + 1])
