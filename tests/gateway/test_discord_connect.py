...
@pytest.mark.asyncio
async def test_post_connect_initialization_skips_fingerprint_after_failed_attempt(tmp_path, monkeypatch):
    adapter = DiscordAdapter(PlatformConfig(enabled=True, token="test-token"))
    ...
    preserved_entry = json.loads(state_path.read_text(encoding="utf-8"))["999"]
    assert preserved_entry["last_success_at"] == 101.0
    assert preserved_entry["summary"] == {"total": 1}
