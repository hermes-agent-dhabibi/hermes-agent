...
class TestYamlFormatting:
    ...
    def test_populated_flow_mapping_is_rejected_without_modifying_file(self, tmp_path):
        """Line insertion must not nest children beneath an inline mapping."""
        original = "terminal: {backend: local}\n"
        (tmp_path / "config.yaml").write_text(original, encoding="utf-8")

        from hermes_cli.config_sync import sync_config

        with pytest.raises(ValueError, match="inline YAML value"):
            sync_config()

        assert (tmp_path / "config.yaml").read_text(encoding="utf-8") == original
        assert yaml.safe_load(original) == {"terminal": {"backend": "local"}}
