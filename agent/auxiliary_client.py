...
def _to_async_client(sync_client: Any, model: str, *, is_vision: bool = False) -> Tuple[Any, str]:
    ...
    preserved_headers = dict(
        getattr(sync_client, "_default_headers", None)
        or getattr(sync_client, "_custom_headers", None)
        or {}
    )
...
def _resolve_strict_vision_backend(provider: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if provider == "copilot":
        ...
        return resolve_provider_client("copilot", model="gpt-5-mini", is_vision=True)
    if provider == "openrouter":
...
# Client cache: (provider, async_mode, base_url, api_key) -> (client, default_model, loop)
# NOTE: loop identity is NOT part of the key. On async cache hits we check
# whether the cached loop is the *current, open* loop; if not, the stale entry
# is replaced in-place. This bounds cache growth to one entry per unique
